(function( $ ) {
	...
	// calculate the totals on the fly when editing or adding a quote or invoice
 	...
 	function workOutTotals(){
        ...
        $('.sliced input.item_amount').each( function() {
			...
			var add_opt = $(group).find('#_sliced_items_' + index + '_additional_option').is(":checked");

			var add_opt_show = $(group).find('#_sliced_items_' + index + '_additional_option_show').is(":checked");
    ...
    $(document).on('change', 'select.pre_defined_products', function () {
        ...
        var emission    = $(this).find(':selected').data('emission');
		...
		// jQuery('#_sliced_items_' + index + '_additional_option').unbind('click');
        jQuery('#_sliced_items_' + index + '_additional_option').prop("checked", (emission == 'Y')).prop("readonly", (emission == 'Y'));
        // jQuery('#_sliced_items_' + index + '_additional_option').click(function(e){
        // 	e.preventDefault();
        // 	return false;
        // })

        //$('#_sliced_items_' + index + '_description').val( desc );
	 	tinyMCE.get('_sliced_items_' + index + '_description').setContent(desc);
	...
	$(document).on('click', '.clsadditional_option_show', function(e) {
		var group   = $(this).parents('.cmb-repeatable-grouping');
        var index   = group.data('iterator');
        if (!$('#_sliced_items_' + index + '_additional_option').is(':checked')) {
			e.preventDefault();
    		return false;
        }
		workOutTotals();
    ...
    $(document).on('click', '.clsadditional_option', function() {
    	var group   = $(this).parents('.cmb-repeatable-grouping');
        var index   = group.data('iterator');
        if (!$(this).is(':checked')) {
			jQuery('#_sliced_items_' + index + '_additional_option_show').prop("checked", false);
        }
		workOutTotals();
    })
    ...
    $(document).on('change', 'select.specifi_emission_products', function () {
    	var specification   = $(this).find(':selected').data('specification');
    	var emission   = $(this).find(':selected').data('emission');
    	$("#_sliced_specification-tmce").click();
		 tinyMCE.get('_sliced_specification').setContent(specification);
    });
