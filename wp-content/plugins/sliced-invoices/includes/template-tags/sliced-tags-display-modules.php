		...
		$output = '<table class="table table-sm table-bordered table-striped">
			...
			<thead>
				...
				<tr>
					...
					<th class="vehicle_type"><strong>' . ( isset( $translate['vehicle_type'] ) ? $translate['vehicle_type'] : __( 'Vehicle Type', 'sliced-invoices') ) . '</strong></th>

					<th class="brand_type"><strong>' . ( isset( $translate['brand_type'] ) ? $translate['brand_type'] : __( 'Brand Type', 'sliced-invoices') ) . '</strong></th>
				...
				foreach ( $items[0] as $item ) {
					...
					$line_total = $shared->get_line_item_sub_total( $shared->get_raw_number( $qty ), $amt, $tax );
						...
						$output .= '<tr class="row_' . $class . ' sliced-item">
							...
							<td class="vehicle_type">' . esc_html__( $vehicle_type ) . '</td>

							<td class="brand_type">' . esc_html__( $brand_type ) . '</td>
