...
function sliced_mpdf_array_splice_reverse(&$arr, $offset, $length) {
	$tmp = (array_reverse(array_slice($arr, $offset, $length)));
	array_splice($arr, $offset, $length, $tmp);
}


function sliced_mpdf_array_insert(&$array, $value, $offset) {
	if (is_array($array)) {
...
// mPDF 5.7.4 URLs
function sliced_mpdf_urldecode_parts($url) {
	$file=$url;
	$query='';
	if (preg_match('/[?]/',$url)) {
...
function _sliced_mpdf_strspn($str1, $str2, $start=null, $length=null) {
...
function _sliced_mpdf_strcspn($str1, $str2, $start=null, $length=null) {
	$numargs = func_num_args();
	if ($numargs == 2) {
...
function _sliced_mpdf_fgets (&$h, $force=false) {
	$startpos = ftell($h);
	$s = fgets($h, 1024);
	if ($force && preg_match("/^([^\r\n]*[\r\n]{1,2})(.)/",trim($s), $ns)) {
		$s = $ns[1];
...
function sliced_mpdf_PreparePreText($text,$ff='//FF//') {
...
if(!function_exists('strcode2utf')){ 
  function strcode2utf($str,$lo=true) {
	...
	// mPDF 5.7
	if ($lo) {
		$str = preg_replace_callback('/\&\#([0-9]+)\;/m', 'sliced_mpdf_code2utf_lo_callback', $str);
		$str = preg_replace_callback('/\&\#x([0-9a-fA-F]+)\;/m', 'sliced_mpdf_codeHex2utf_lo_callback', $str);
	}
	else {
		$str = preg_replace_callback('/\&\#([0-9]+)\;/m', 'sliced_mpdf_code2utf_callback', $str);
		$str = preg_replace_callback('/\&\#x([0-9a-fA-F]+)\;/m', 'sliced_mpdf_codeHex2utf_callback', $str);
	}
	return $str;
  }
}
function sliced_mpdf_code2utf_callback($matches) {
	return code2utf($matches[1], 0);
}
function sliced_mpdf_code2utf_lo_callback($matches) {
	return code2utf($matches[1], 1);
}
function sliced_mpdf_codeHex2utf_callback($matches) {
	return codeHex2utf($matches[1], 0);
}
function sliced_mpdf_codeHex2utf_lo_callback($matches) {
