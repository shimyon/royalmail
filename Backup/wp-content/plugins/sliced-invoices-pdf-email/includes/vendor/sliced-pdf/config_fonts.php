...
// make sure we're in a WP context
if ( ! function_exists( 'get_option' ) ) {
	// if we're here, we must be calling sliced-pdf outside of WP (maybe for debugging?)
	// if so, we need to set up a couple of things
	// this first one is a guess and won't always work, but hey it's worth a try:
	define( '_SLICED_MPDF_SYSTEM_TTFONTS', _SLICED_MPDF_PATH . '../../../../sliced-invoices/public/fonts/' );
	define( 'WP_USE_THEMES', false );
	// another guess that won't always work:
	require( _SLICED_MPDF_PATH . '../../../../../../wp-load.php' );
}

// do we have an extra font to deal with?
$extra_font = false;
$extra_font_ext = false;
$pdf_options = get_option( 'sliced_pdf' );
if ( $pdf_options ) {
	if ( ! empty ( $pdf_options['extra_font_id'] ) ) {
		$extra_font = get_attached_file( $pdf_options['extra_font_id'] );
	}
	if ( ! empty( $pdf_options['extra_font_ext_id'] ) ) {
		$extra_font_ext = get_attached_file( $pdf_options['extra_font_ext_id'] );
	}
}

// Optionally define a folder which contains TTF fonts
// mPDF will look here before looking in the usual _SLICED_MPDF_TTFONTPATH
// Useful if you already have a folder for your fonts
// e.g. on Windows: define("_SLICED_MPDF_SYSTEM_TTFONTS", 'C:/Windows/Fonts/');

// this has been moved into the public class file
//if ( !defined("_SLICED_MPDF_SYSTEM_TTFONTS")) { define("_SLICED_MPDF_SYSTEM_TTFONTS",  require_once plugin_dir_path( __FILE__ ) . 'includes/fonts/' ); }
...
$this->backupSubsFont = array('dejavusanscondensed', 'dejavusans','arialunicodems','extra_font');	// this will recognise most scripts
...
$this->backupSIPFont = 'extra_font_ext';
...
if ( $extra_font && $extra_font_ext ) {
	$fontdata['extra_font'] = array(
		'R' => $extra_font,
		'sip-ext' => 'extra_font_ext'
	);
	$fontdata['extra_font_ext'] = array(
		'R' => $extra_font_ext
	);
} elseif ( $extra_font ) {
	$fontdata['extra_font'] = array(
