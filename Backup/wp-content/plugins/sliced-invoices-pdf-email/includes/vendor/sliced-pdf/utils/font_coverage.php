...
$mpdf=new Sliced_mPDF('','A4-L','','',10,10,10,10); 
...
for ($urgp = 0; $urgp < $nofgroups; $urgp++) {
  ...
  foreach ($tempfontdata AS $fname => $v) {
	$cw = '';
	if (file_exists((_SLICED_MPDF_TTFONTDATAPATH.$fname.'.cw.dat'))) { $cw = file_get_contents(_SLICED_MPDF_TTFONTDATAPATH.$fname.'.cw.dat'); }
	else {
		$mpdf->fontdata[$fname]['R'] = $tempfontdata[$fname]['file']; 
		$mpdf->AddFont($fname);
		$cw = file_get_contents(_SLICED_MPDF_TTFONTDATAPATH.$fname.'.cw.dat'); 
	}
	if (!$cw) {
		continue;
