...
//==============================================================
	...
	//include(_SLICED_MPDF_TTFONTDATAPATH.$fontkey.'.mtx.php');

	$ttffile = '';
	if (defined('_SLICED_MPDF_SYSTEM_TTFONTS')) {
		$ttffile = _SLICED_MPDF_SYSTEM_TTFONTS.$mpdf->fontdata[$family][$stylekey];
		if (!file_exists($ttffile)) { $ttffile = ''; }
	}
	if (!$ttffile) {
		$ttffile = _SLICED_MPDF_TTFONTPATH.$mpdf->fontdata[$family][$stylekey];
	...
	include(_SLICED_MPDF_PATH .'classes/otl_dump.php');
	$ttf = new Sliced_OTLdump($mpdf);
