...
function applyOTL($str, $useOTL) {
	...
	if (!isset($this->GDEFdata[$this->fontkey])) {
		include(_SLICED_MPDF_TTFONTDATAPATH.$this->fontkey.'.GDEFdata.php'); 
	...
	foreach($earr as $char) {
		$ucd_record = Sliced_UCDN::get_ucd_record($char);
		$sbl = $ucd_record[6];

		// Special case - Arabic End of Ayah
		if ($char==1757) { $sbl = Sliced_UCDN::SCRIPT_ARABIC; }
...
for($sch=0;$sch<=$subchunk;$sch++) {
	...
	if (Sliced_UCDN::SCRIPT_DEVANAGARI <= $scriptblock && $scriptblock <= Sliced_UCDN::SCRIPT_MALAYALAM) { $this->shaper = "I"; }	// INDIC shaper
	else if ($scriptblock == Sliced_UCDN::SCRIPT_ARABIC || $scriptblock == Sliced_UCDN::SCRIPT_SYRIAC) { $this->shaper = "A"; }	// ARABIC shaper
	else if ($scriptblock == Sliced_UCDN::SCRIPT_NKO || $scriptblock == Sliced_UCDN::SCRIPT_MANDAIC) { $this->shaper = "A"; }	// ARABIC shaper
	else if ($scriptblock == Sliced_UCDN::SCRIPT_KHMER) { $this->shaper = "K"; }	// KHMER shaper
	else if ($scriptblock == Sliced_UCDN::SCRIPT_THAI) { $this->shaper = "T"; }	// THAI shaper
	else if ($scriptblock == Sliced_UCDN::SCRIPT_LAO) { $this->shaper = "L"; }	// LAO shaper
	else if ($scriptblock == Sliced_UCDN::SCRIPT_SINHALA) { $this->shaper = "S"; }	// SINHALA shaper
	else if ($scriptblock == Sliced_UCDN::SCRIPT_MYANMAR) { $this->shaper = "M"; }	// MYANMAR shaper
	else if ($scriptblock == Sliced_UCDN::SCRIPT_NEW_TAI_LUE) { $this->shaper = "E"; }	// SEA South East Asian shaper
	else if ($scriptblock == Sliced_UCDN::SCRIPT_CHAM) { $this->shaper = "E"; }		// SEA South East Asian shaper
	else if ($scriptblock == Sliced_UCDN::SCRIPT_TAI_THAM) { $this->shaper = "E"; }	// SEA South East Asian shaper
	else $this->shaper = "";
	// Get scripttag based on actual text script
	$scripttag = Sliced_UCDN::$uni_scriptblock[$scriptblock];
	...
	if (!isset($this->GDEFdata[$this->fontkey]['GSUBGPOStables'])) {
		$this->ttfOTLdata = $this->GDEFdata[$this->fontkey]['GSUBGPOStables'] = file_get_contents(_SLICED_MPDF_TTFONTDATAPATH.$this->fontkey.'.GSUBGPOStables.dat','rb') or die('Can\'t open file ' . _SLICED_MPDF_TTFONTDATAPATH.$this->fontkey.'.GSUBGPOStables.dat');
	}
	else {
...
////////////////////////////////////////////////////////////////
	...
	else if ($this->mpdf->useTibetanLBR && $scriptblock == Sliced_UCDN::SCRIPT_TIBETAN ) {
		// Sets $this->OTLdata[$i]['wordend']=true at possible end of word boundaries
...
////////////////////////////////////////////////////////////////
  if (($useOTL & 0xFF) && $GSUBscriptTag && $GSUBlangsys && $GSUBFeatures) {
	...
	if (!isset($this->GSUBdata[$this->GSUBfont])) {
		if (file_exists(_SLICED_MPDF_TTFONTDATAPATH.$this->mpdf->CurrentFont['fontkey'].'.GSUB.'.$GSUBscriptTag.'.'.$GSUBlangsys.'.php')) {
			include_once(_SLICED_MPDF_TTFONTDATAPATH.$this->mpdf->CurrentFont['fontkey'].'.GSUB.'.$GSUBscriptTag.'.'.$GSUBlangsys.'.php'); 
	...
	if (!isset($this->GSUBdata[$this->fontkey])) {
		include(_SLICED_MPDF_TTFONTDATAPATH.$this->fontkey.'.GSUBdata.php'); 
	...
	if ($this->shaper == 'A') {
		...
		//-----------------------------------------------------------------------------------
		//if ($scriptblock == Sliced_UCDN::SCRIPT_ARABIC ) {
		for ($i=0;$i<count($this->OTLdata);$i++) {
		...
		for($ptr=1; $ptr<count($this->OTLdata); $ptr++) {
			if ($this->OTLdata[$ptr]['general_category'] == Sliced_UCDN::UNICODE_GENERAL_CATEGORY_NON_SPACING_MARK && $this->OTLdata[$ptr-1]['uni'] == 32) {
	...
	//===================================
		...
		for($ptr=0; $ptr<count($this->OTLdata); $ptr++) {
			...
			$sub = Sliced_INDIC::decompose_indic($char);
			if ($sub) {
				$newinfo = array();
				for($i=0;$i<count($sub);$i++) {
					$newinfo[$i] = array();
					$ucd_record = Sliced_UCDN::get_ucd_record($sub[$i]);
			...
			if ($this->shaper == 'I') {
			  if ($char == 0x09AF && isset($this->OTLdata[$ptr + 1]) && $this->OTLdata[$ptr + 1]['uni'] == 0x09BC) { 
				...
				$newinfo = array();
				$newinfo[0] = array();
				$ucd_record = Sliced_UCDN::get_ucd_record($sub);
		...
		foreach($this->OTLdata AS $eid=>$c) {
			Sliced_INDIC::set_indic_properties($this->OTLdata[$eid], $scriptblock );	// sets ['indic_category'] and ['indic_position']
			//$c['general_category']
			//$c['combining_class']
			//$c['uni'] =  $char;

			$indic_category_string .= Sliced_INDIC::$indic_category_char[$this->OTLdata[$eid]['indic_category']];
		}

		$broken_syllables = false;
		if ($this->shaper == 'I') {
			Sliced_INDIC::set_syllables($this->OTLdata, $indic_category_string, $broken_syllables);
		}
		else if ($this->shaper == 'S') {
			Sliced_INDIC::set_syllables_sinhala($this->OTLdata, $indic_category_string, $broken_syllables);
		}
		else if ($this->shaper == 'K') {
			Sliced_INDIC::set_syllables_khmer($this->OTLdata, $indic_category_string, $broken_syllables);
		}
		$indic_category_string = '';
		...
		$indic_config = Sliced_INDIC::$indic_configs[$scriptblock];
		$dottedcircle = false; 
		if ($broken_syllables) { 
			if ($this->mpdf->_charDefined($this->mpdf->fonts[$this->fontkey]['cw'],0x25CC) ) { 
				$dottedcircle = array();
				$ucd_record = Sliced_UCDN::get_ucd_record(0x25CC);
				...
				$dottedcircle[0]['indic_category'] = Sliced_INDIC::OT_DOTTEDCIRCLE;
				$dottedcircle[0]['indic_position'] = Sliced_INDIC::POS_BASE_C;

				$dottedcircle[0]['hex'] =  '025CC';		// TEMPORARY *****
			}
		}
		Sliced_INDIC::initial_reordering($this->OTLdata, $this->GSUBdata[$this->GSUBfont], $broken_syllables, $indic_config, $scriptblock, $is_old_spec, $dottedcircle);
		...
		Sliced_INDIC::final_reordering($this->OTLdata, $this->GSUBdata[$this->GSUBfont], $indic_config, $scriptblock, $is_old_spec);
	...
	// NB Old style 'mymr' is left to go through the default shaper
		...
		foreach($this->OTLdata AS $eid=>$c) {
			Sliced_MYANMAR::set_myanmar_properties($this->OTLdata[$eid]);	// sets ['myanmar_category'] and ['myanmar_position']
			$myanmar_category_string .= Sliced_MYANMAR::$myanmar_category_char[$this->OTLdata[$eid]['myanmar_category']];
		}
		$broken_syllables = false;
		Sliced_MYANMAR::set_syllables($this->OTLdata, $myanmar_category_string, $broken_syllables);
		$myanmar_category_string = '';
		...
		if ($broken_syllables) { 
			if ($this->mpdf->_charDefined($this->mpdf->fonts[$this->fontkey]['cw'],0x25CC) ) { 
				...
				$dottedcircle[0]['group'] =  'C';
				$dottedcircle[0]['uni'] =  0x25CC;
				$dottedcircle[0]['myanmar_category'] = Sliced_MYANMAR::OT_DOTTEDCIRCLE;
				$dottedcircle[0]['myanmar_position'] = Sliced_MYANMAR::POS_BASE_C;
				$dottedcircle[0]['hex'] =  '025CC';
			}
		}
		Sliced_MYANMAR::reordering($this->OTLdata, $this->GSUBdata[$this->GSUBfont], $broken_syllables, $dottedcircle);
	...
	//==============================
       ...
       * GSUB/GPOS needed, so there may be no scripts found! */
		...
		$sea_category_string = '';
		foreach($this->OTLdata AS $eid=>$c) {
			Sliced_SEA::set_sea_properties($this->OTLdata[$eid], $scriptblock );	// sets ['sea_category'] and ['sea_position']
			//$c['general_category']
			//$c['combining_class']
			//$c['uni'] =  $char;

			$sea_category_string .= Sliced_SEA::$sea_category_char[$this->OTLdata[$eid]['sea_category']];
		}

		$broken_syllables = false;
		Sliced_SEA::set_syllables($this->OTLdata, $sea_category_string, $broken_syllables);
		$sea_category_string = '';
		...
		if ($broken_syllables) { 
			if ($this->mpdf->_charDefined($this->mpdf->fonts[$this->fontkey]['cw'],0x25CC) ) { 
				...
				$dottedcircle[0]['group'] =  'C';
				$dottedcircle[0]['uni'] =  0x25CC;
				$dottedcircle[0]['sea_category'] = Sliced_SEA::OT_GB;
				$dottedcircle[0]['sea_position'] = Sliced_SEA::POS_BASE_C;

				$dottedcircle[0]['hex'] =  '025CC';		// TEMPORARY *****
			}
		}
		Sliced_SEA::initial_reordering($this->OTLdata, $this->GSUBdata[$this->GSUBfont], $broken_syllables, $scriptblock, $dottedcircle);
		...
		Sliced_SEA::final_reordering($this->OTLdata, $this->GSUBdata[$this->GSUBfont], $scriptblock);
	...
	//==============================
		...
		if ($this->shaper == 'T' || $this->shaper == 'L') {
			for($ptr=0; $ptr<count($this->OTLdata); $ptr++) {
				$char = $this->OTLdata[$ptr]['uni'];
					...
					$newinfo = array();
					$ucd_record = Sliced_UCDN::get_ucd_record($sub[0]);
					$newinfo[0]['general_category'] = $ucd_record[0];
					$newinfo[0]['bidi_type'] = $ucd_record[2];
					...
					$newinfo = array();
					$ucd_record = Sliced_UCDN::get_ucd_record($sub[1]);
					$newinfo[0]['general_category'] = $ucd_record[0];
					$newinfo[0]['bidi_type'] = $ucd_record[2];
		...
		if ($scriptblock == Sliced_UCDN::SCRIPT_TIBETAN) {
			// =========================
			// Reordering TIBETAN
			// =========================
			...
			for($ptr=0; $ptr<count($this->OTLdata)-1; $ptr++) {
    				if (Sliced_INDIC::in_range($this->OTLdata[$ptr]['uni'], 0x0F20, 0x0F33) && $this->OTLdata[$ptr+1]['uni'] == 0x0F3F ) {
...
*/
		...
		*/
		...
		if (!(($this->mpdf->textvar & SLICED_FC_SMALLCAPS) && $scriptblock == Sliced_UCDN::SCRIPT_LATIN && strpos($useGSUBtags, 'smcp')===false)) {
	...
	//=======================================================
	if (($this->shaper == "K" || $this->shaper == "T" || $this->shaper == "L") || $scriptblock == Sliced_UCDN::SCRIPT_TIBETAN ) {
		...
		$newinfo[0] = array(
			'general_category' => Sliced_UCDN::UNICODE_GENERAL_CATEGORY_FORMAT, 
			'bidi_type' => Sliced_UCDN::BIDI_CLASS_BN, 
...
////////////////////////////////////////////////////////////////
  ...
  if (($useOTL & 0xFF) && $GPOSscriptTag && $GPOSlangsys && $GPOSFeatures) {
	...
	if (!isset($this->GPOSdata[$this->fontkey])) {
		include(_SLICED_MPDF_TTFONTDATAPATH.$this->mpdf->CurrentFont['fontkey'].'.GPOSdata.php'); 
	...
	if ($scriptblock != Sliced_UCDN::SCRIPT_LATIN) { $tags .= ' kern'; }
				...
				foreach($this->GSUBLookups[$lu]['Subtables'] AS $c=>$subtable_offset) {
					...
					if (isset($this->GSLuCoverage[$lu][$c][$currGID])) {
						if (strpos('rphf pref blwf half pstf cfar init' , $usetag)!==false) { // only apply when mask indicates
							$mask = 0;
							switch ($usetag) {
								case 'rphf': $mask = (1<<(Sliced_INDIC::RPHF)); break;
								case 'pref': $mask = (1<<(Sliced_INDIC::PREF)); break;
								case 'blwf': $mask = (1<<(Sliced_INDIC::BLWF)); break;
								case 'half': $mask = (1<<(Sliced_INDIC::HALF)); break;
								case 'pstf': $mask = (1<<(Sliced_INDIC::PSTF)); break;
								case 'cfar': $mask = (1<<(Sliced_INDIC::CFAR)); break;
								case 'init': $mask = (1<<(Sliced_INDIC::INIT)); break;
							}
...
function GSUBsubstitute($pos, $substitute, $Type, $GlyphPos=NULL ) {
	...
	// LookupType 2: Multiple Substitution Subtable : 1 to n
		...
		//	$bt = Sliced_UCDN::get_bidi_class($uni);
		//	if (!$bt) { 
...
/*
   ...
   */
		...
		// Get types of new inserted chars - or replicate type of char being replaced
	//	$bt = Sliced_UCDN::get_bidi_class($substitute);
	//	if (!$bt) { 
			$bt = $this->OTLdata[$pos]['bidi_type']; 
	//	}
...
function SEAlineBreaking() {
	...
	if (!isset($this->lbdicts[$this->shaper]) && file_exists(_SLICED_MPDF_PATH.'includes/linebrdict'.$this->shaper.'.dat')) { $this->lbdicts[$this->shaper] = file_get_contents(_SLICED_MPDF_PATH.'includes/linebrdict'.$this->shaper.'.dat'); }
...
function _bidiSort($ta, $str='', $dir, &$chunkOTLdata, $useGPOS) {
	...
	for ($i=0; $i < $numchars; ++$i) {
		...
		$chardata[$i]['sor'] = max($left, $level) % 2 ? Sliced_UCDN::BIDI_CLASS_R : Sliced_UCDN::BIDI_CLASS_L;
		$chardata[$i]['eor'] = max($right, $level) % 2 ? Sliced_UCDN::BIDI_CLASS_R : Sliced_UCDN::BIDI_CLASS_L;
	}
	...
	for ($i=0; $i < $numchars; ++$i) {
		if ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_NSM) {
	...
	$levcount = 0;
	for ($i=0; $i < $numchars; ++$i) {
		if ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_EN) {
			$found = false;
			for ($j=$levcount; $j >= 0; $j--) {
				if ($chardata[$j]['type'] == Sliced_UCDN::BIDI_CLASS_AL) { $chardata[$i]['type'] = Sliced_UCDN::BIDI_CLASS_AN; $found = true; break; }
				else if (($chardata[$j]['type'] == Sliced_UCDN::BIDI_CLASS_L) || ($chardata[$j]['type'] == Sliced_UCDN::BIDI_CLASS_R)) { $found = true; break; }
			}
		}
	...
	for ($i=0; $i < $numchars; ++$i) {
 		if ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_AL) { $chardata[$i]['type'] = Sliced_UCDN::BIDI_CLASS_R; }
	}
	...
	for ($i=1; $i < $numchars; ++$i) {
		if ( ($i+1) < $numchars && $chardata[($i)]['level'] == $chardata[($i+1)]['level'] && $chardata[($i)]['level'] == $chardata[($i-1)]['level']) {
			if ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_ES && $chardata[($i-1)]['type'] == Sliced_UCDN::BIDI_CLASS_EN && $chardata[($i+1)]['type'] == Sliced_UCDN::BIDI_CLASS_EN) {
				$chardata[$i]['type'] = Sliced_UCDN::BIDI_CLASS_EN;
			}
			else if ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_CS && $chardata[($i-1)]['type'] == Sliced_UCDN::BIDI_CLASS_EN && $chardata[($i+1)]['type'] == Sliced_UCDN::BIDI_CLASS_EN) {
				$chardata[$i]['type'] = Sliced_UCDN::BIDI_CLASS_EN;
			}
			else if ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_CS && $chardata[($i-1)]['type'] == Sliced_UCDN::BIDI_CLASS_AN && $chardata[($i+1)]['type'] == Sliced_UCDN::BIDI_CLASS_AN) {
				$chardata[$i]['type'] = Sliced_UCDN::BIDI_CLASS_AN;
			}
		}
	}
	...
	for ($i=0; $i < $numchars; ++$i) {
		if ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_ET) {
			if ($i > 0 && $chardata[($i-1)]['type'] == Sliced_UCDN::BIDI_CLASS_EN && $chardata[($i)]['level'] == $chardata[($i-1)]['level']) {
				$chardata[$i]['type'] = Sliced_UCDN::BIDI_CLASS_EN;
			}
			else {
				$j = $i+1;
				while ($j < $numchars && $chardata[$j]['level'] == $chardata[$i]['level'] ) {
					if ($chardata[$j]['type'] == Sliced_UCDN::BIDI_CLASS_EN) {
						$chardata[$i]['type'] = Sliced_UCDN::BIDI_CLASS_EN;
						break;
					}
					else if ($chardata[$j]['type'] != Sliced_UCDN::BIDI_CLASS_ET) { break; }
					++$j;
				}
			}
		}
	...
	for ($i=0; $i < $numchars; ++$i) {
		if (($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_ET) || ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_ES) || ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_CS)) {
			$chardata[$i]['type'] = Sliced_UCDN::BIDI_CLASS_ON;
		}
	}
	...
	for ($i=0; $i < $numchars; ++$i) {
		if ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_EN) {
			if ($i==0) {	// Start of Level run
				if ($chardata[$i]['sor']==Sliced_UCDN::BIDI_CLASS_L) $chardata[$i]['type'] = $chardata[$i]['sor'];
			}
			else {
				for ($j=$i-1; $j >= 0; $j--) {
					if ($chardata[$j]['level'] != $chardata[$i]['level']) {	// Level run boundary 
						if ($chardata[$j+1]['sor']==Sliced_UCDN::BIDI_CLASS_L) $chardata[$i]['type'] = $chardata[$j+1]['sor'];
						break;
					}
					else if ($chardata[$j]['type'] == Sliced_UCDN::BIDI_CLASS_L) {
						$chardata[$i]['type'] = Sliced_UCDN::BIDI_CLASS_L;
						break;
					}
					else if ($chardata[$j]['type'] == Sliced_UCDN::BIDI_CLASS_R) {
						break;
					}
				}
			}
	...
	for ($i=0; $i < $numchars; ++$i) {
		if ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_ON || $chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_WS) {
			$left = -1;
			// LEFT
			if ($i==0) { 	// first char 
				$left = $chardata[($i)]['sor'];
			...
			else if ($chardata[($i-1)]['type'] == Sliced_UCDN::BIDI_CLASS_L) {
				$left = Sliced_UCDN::BIDI_CLASS_L;
			}
			else if ($chardata[($i-1)]['type'] == Sliced_UCDN::BIDI_CLASS_R || $chardata[($i-1)]['type'] == Sliced_UCDN::BIDI_CLASS_EN || $chardata[($i-1)]['type'] == Sliced_UCDN::BIDI_CLASS_AN) {
				$left = Sliced_UCDN::BIDI_CLASS_R;
			}
			// RIGHT
			$right = -1;
			$j=$i;
			// move to the right of any following neutrals OR hit a run boundary
			while(($chardata[$j]['type'] == Sliced_UCDN::BIDI_CLASS_ON || $chardata[$j]['type'] == Sliced_UCDN::BIDI_CLASS_WS) && $j<=($numchars-1)) { 
				...
				else if ($chardata[($j+1)]['type'] == Sliced_UCDN::BIDI_CLASS_L) {
					$right = Sliced_UCDN::BIDI_CLASS_L;
					break;
				}
				else if ($chardata[($j+1)]['type'] == Sliced_UCDN::BIDI_CLASS_R || $chardata[($j+1)]['type'] == Sliced_UCDN::BIDI_CLASS_EN || $chardata[($j+1)]['type'] == Sliced_UCDN::BIDI_CLASS_AN) {
					$right = Sliced_UCDN::BIDI_CLASS_R;
					break;
				}
				$j++; 
			}
	...
	for ($i=0; $i < $numchars; ++$i) {
		if ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_ON || $chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_WS) {
			$chardata[$i]['type'] = ($chardata[$i]['level'] % 2) ?  Sliced_UCDN::BIDI_CLASS_R : Sliced_UCDN::BIDI_CLASS_L;
			$chardata[$i]['orig_type'] = $chardata[$i]['type'];	// Need to store the original 'WS' for reference in L1 below
		}
	}
	...
	for ($i=0; $i < $numchars; ++$i) {
		...
		if ($odd) {
			if (($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_L) || ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_AN) || ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_EN)) {
				$chardata[$i]['level'] += 1;
			}
		}
		else {
			if ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_R) { $chardata[$i]['level'] += 1; }
			else if (($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_AN) || ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_EN)) { $chardata[$i]['level'] += 2; }
		}
...
//
	...
	for ($i=($numchars-1); $i>0; $i--) {
		if ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_WS || (isset($chardata[$i]['orig_type']) && $chardata[$i]['orig_type'] == Sliced_UCDN::BIDI_CLASS_WS)) {
	...
	for ($j=$maxlevel; $j > 0; $j--) {
		...
		for ($i=0; $i < $numchars; ++$i) {
			if ($chardata[$i]['level'] >= $j) {
				...
				if (isset(Sliced_UCDN::$mirror_pairs[$chardata[$i]['char']]) && $chardata[$i]['type']==Sliced_UCDN::BIDI_CLASS_R) {
					$chardata[$i]['char'] = Sliced_UCDN::$mirror_pairs[$chardata[$i]['char']];
				}
	...
	foreach ($chardata as $cd) {
		...
		if($cd['type']==Sliced_UCDN::BIDI_CLASS_L) { $rtl_content |= 1; }
		else if($cd['type']==Sliced_UCDN::BIDI_CLASS_R) { $rtl_content |= 2; }
		$cctr++;
	}
...
function _bidiPrepare(&$para, $dir) {
		  ...
		  if ($chunkOTLdata['char_data'][$i]['uni'] == 8235) {	// RLE
			...
			if ($chunkOTLdata['char_data'][$i]['uni'] == 8296) {	// FSI
				...
				while (!($nc2==($numchunks-1) && $i2==((count($para[$nc2][18]['char_data']))-1))) { 	// while not at end of last chunk
					...
					if ($para[$nc2][18]['char_data'][$i2]['bidi_class'] === Sliced_UCDN::BIDI_CLASS_L || $para[$nc2][18]['char_data'][$i2]['bidi_class'] == Sliced_UCDN::BIDI_CLASS_AL || $para[$nc2][18]['char_data'][$i2]['bidi_class'] === Sliced_UCDN::BIDI_CLASS_R) {
				...
				if (!$fsi && $fsi!==0) {
					if ($pel==1) { $fsi = Sliced_UCDN::BIDI_CLASS_R ; }
					else { $fsi = Sliced_UCDN::BIDI_CLASS_L ; }
				}
			}

			if ($chunkOTLdata['char_data'][$i]['uni'] == 8294 || $fsi === Sliced_UCDN::BIDI_CLASS_L ) {	// LRI or FSI-L
			...
			else if ($chunkOTLdata['char_data'][$i]['uni'] == 8295 || $fsi == Sliced_UCDN::BIDI_CLASS_R || $fsi == Sliced_UCDN::BIDI_CLASS_AL ) {	// RLI or FSI-R
			...
			if ($dos != -1) { $chardir = $dos; } 
				...
				$chardir = $chunkOTLdata['char_data'][$i]['bidi_class'];
				if ($chardir == Sliced_UCDN::BIDI_CLASS_R || $chardir == Sliced_UCDN::BIDI_CLASS_AL) { $strongrtl = true; }
			}
			$chunkOTLdata['char_data'][$i]['level'] = $cel;
			$chunkOTLdata['char_data'][$i]['type'] = $chardir;
			$chunkOTLdata['char_data'][$i]['diid'] = $diid;
		...
		for ($nc=0;$nc<$numchunks;$nc++) {
			...
			for ($i=0; $i < $numchars; ++$i) {
				...
				if ($firstchar || $level!=$prelevel) {
					$chardata[$i]['sor'] = max($prelevel, $level) % 2 ? Sliced_UCDN::BIDI_CLASS_R : Sliced_UCDN::BIDI_CLASS_L;
				}
	...
	for ($ir=0; $ir<=$dictr;$ir++) {
		...
		for ($nc=0;$nc<$numchunks;$nc++) {
			...
			for ($i=0; $i < $numchars; ++$i) {
				...
				if ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_EN && $laststrongtype == Sliced_UCDN::BIDI_CLASS_AL ) { 
					$chardata[$i]['type'] = Sliced_UCDN::BIDI_CLASS_AN; 
				}
				if ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_L || $chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_R || $chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_AL) { 
	...
	for ($nc=0;$nc<$numchunks;$nc++) {
		...
		$numchars = count($chardata);
		for ($i=0; $i < $numchars; ++$i) {
 			if (isset($chardata[$i]['type']) && $chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_AL) { $chardata[$i]['type'] = Sliced_UCDN::BIDI_CLASS_R; }
		}
	}
		...
		for ($nc=0;$nc<$numchunks;$nc++) {
			...
			for ($i=0; $i < $numchars; ++$i) {
				...
				if (!isset($chardata[$i]['sor']) && !isset($chardata[$i]['eor'])) {
					if ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_ES && $prevtype == Sliced_UCDN::BIDI_CLASS_EN && $nexttype == Sliced_UCDN::BIDI_CLASS_EN) {
						$chardata[$i]['type'] = Sliced_UCDN::BIDI_CLASS_EN;
					}
					else if ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_CS && $prevtype == Sliced_UCDN::BIDI_CLASS_EN && $nexttype == Sliced_UCDN::BIDI_CLASS_EN) {
						$chardata[$i]['type'] = Sliced_UCDN::BIDI_CLASS_EN;
					}
					else if ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_CS && $prevtype == Sliced_UCDN::BIDI_CLASS_AN && $nexttype == Sliced_UCDN::BIDI_CLASS_AN) {
						$chardata[$i]['type'] = Sliced_UCDN::BIDI_CLASS_AN;
					}
				}
				$prevtype = $chardata[$i]['type'];
	...
	for ($ir=0; $ir<=$dictr;$ir++) {
		...
		for ($nc=0;$nc<$numchunks;$nc++) {
			...
			for ($i=0; $i < $numchars; ++$i) {
				...
				if ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_ET) {
					if ($prevtype == Sliced_UCDN::BIDI_CLASS_EN) {
						$chardata[$i]['type'] = Sliced_UCDN::BIDI_CLASS_EN;
					}
						...
						while (!($nc2==($numchunks-1) && $i2==((count($para[$nc2][18]['char_data']))-1))) { // while not at end of last chunk
							...
							if ($nexttype == Sliced_UCDN::BIDI_CLASS_EN) {
								$chardata[$i]['type'] = Sliced_UCDN::BIDI_CLASS_EN;
								break;
							}
							else if ($nexttype != Sliced_UCDN::BIDI_CLASS_ET) { break; }
						}
					}
				}
				$prevtype = $chardata[$i]['type'];
	...
	for ($nc=0;$nc<$numchunks;$nc++) {
		...
		$numchars = count($chardata);
		for ($i=0; $i < $numchars; ++$i) {
			if (isset($chardata[$i]['type']) && (($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_ET) || ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_ES) || ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_CS))) {
				$chardata[$i]['type'] = Sliced_UCDN::BIDI_CLASS_ON;
			}
		}
	}
	...
	for ($ir=0; $ir<=$dictr;$ir++) {
		...
		for ($nc=0;$nc<$numchunks;$nc++) {
			...
			for ($i=0; $i < $numchars; ++$i) {
				if (!isset($chardata[$i]['diid']) || $chardata[$i]['diid']!=$ir) { continue; }	// Ignore characters in a different isolate run
				if (isset($chardata[$i]['sor'])) { $laststrongtype = $chardata[$i]['sor']; }
				if (isset($chardata[$i]['type']) && $chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_EN && $laststrongtype == Sliced_UCDN::BIDI_CLASS_L ) { 
					$chardata[$i]['type'] = Sliced_UCDN::BIDI_CLASS_L; 
				}
				if (isset($chardata[$i]['type']) && ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_L || $chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_R || $chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_AL)) { 
					$laststrongtype =  $chardata[$i]['type']; 
				}
			}
		}
		...
		for ($nc=0;$nc<$numchunks;$nc++) {
			...
			for ($i=0; $i < $numchars; ++$i) {
				...
				if ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_ON || $chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_WS) {
					$left = -1;
					// LEFT
					if ($laststrongtype == Sliced_UCDN::BIDI_CLASS_R || $laststrongtype == Sliced_UCDN::BIDI_CLASS_EN || $laststrongtype == Sliced_UCDN::BIDI_CLASS_AN) { 
						$left = Sliced_UCDN::BIDI_CLASS_R; 
					}
					else if ($laststrongtype == Sliced_UCDN::BIDI_CLASS_L) { 
						$left = Sliced_UCDN::BIDI_CLASS_L; 
					}
					// RIGHT
					$right = -1;
					...
					if (isset($chardata[$i]['eor'])) { 
						...
						while (!($nc2==($numchunks-1) && $i2==((count($para[$nc2][18]['char_data']))-1))) { // while not at end of last chunk
							...
							$nexttype = $para[$nc2][18]['char_data'][$i2]['type']; 
							if ($nexttype == Sliced_UCDN::BIDI_CLASS_R || $nexttype == Sliced_UCDN::BIDI_CLASS_EN || $nexttype == Sliced_UCDN::BIDI_CLASS_AN) { 
								$right =  Sliced_UCDN::BIDI_CLASS_R; 
								break;
							}
							else if ($nexttype == Sliced_UCDN::BIDI_CLASS_L) { 
								$right =  Sliced_UCDN::BIDI_CLASS_L; 
								break;
							}
				...
				else if ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_L || $chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_R || $chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_EN || $chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_AN) { 
					$laststrongtype =  $chardata[$i]['type']; 
				}
			}
		}
	...
	for ($nc=0;$nc<$numchunks;$nc++) {
		...
		$numchars = count($chardata);
		for ($i=0; $i < $numchars; ++$i) {
			if (isset($chardata[$i]['type']) && ($chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_ON || $chardata[$i]['type'] == Sliced_UCDN::BIDI_CLASS_WS)) {
				$chardata[$i]['orig_type'] = $chardata[$i]['type'];	// Need to store the original 'WS' for reference in L1 below
				$chardata[$i]['type'] = ($chardata[$i]['level'] % 2) ?  Sliced_UCDN::BIDI_CLASS_R : Sliced_UCDN::BIDI_CLASS_L;
			}
		}
...
function _bidiReorder(&$chunkorder, &$content, &$cOTLdata, $blockdir) {
	...
	for ($i=($numchars-1); $i>0; $i--) {
		if ($bidiData[$i]['type'] == Sliced_UCDN::BIDI_CLASS_WS || (isset($bidiData[$i]['orig_type']) && $bidiData[$i]['orig_type'] == Sliced_UCDN::BIDI_CLASS_WS)) {
	...
	for ($j=$maxlevel; $j > 0; $j--) {
		...
		for ($i=0; $i < $numchars; ++$i) {
			if ($bidiData[$i]['level'] >= $j) {
				$onlevel = true;
				// L4. A character is depicted by a mirrored glyph if and only if (a) the resolved directionality of that character is R, and (b) the Bidi_Mirrored property value of that character is true. 
				if (isset(Sliced_UCDN::$mirror_pairs[$bidiData[$i]['uni']]) && $bidiData[$i]['type']==Sliced_UCDN::BIDI_CLASS_R) {
					$bidiData[$i]['uni'] = Sliced_UCDN::$mirror_pairs[$bidiData[$i]['uni']];
				}
...
function _getOTLscriptTag($ScriptLang, $scripttag, $scriptblock, $shaper, $useOTL, $mode) {
	...
	// $scriptblock is the (number/code) for the script of the actual text string based on Unicode properties (Sliced_UCDN::$uni_scriptblock)
...
*/
	...
	if ($scriptblock == Sliced_UCDN::SCRIPT_LATIN) {
		if (!($useOTL & 0x01)) { return array('',false); }
	}
	else if ($scriptblock == Sliced_UCDN::SCRIPT_CYRILLIC) {
		if (!($useOTL & 0x02)) { return array('',false); }
	}
	else if ($scriptblock == Sliced_UCDN::SCRIPT_GREEK) {
		if (!($useOTL & 0x04)) { return array('',false); }
	}
	else if ($scriptblock >= Sliced_UCDN::SCRIPT_HIRAGANA && $scriptblock <= Sliced_UCDN::SCRIPT_YI ) { 
...
function _getOTLLangTag($ietf, $available) {
	...
	if ($lang!='' && isset(Sliced_UCDN::$ot_languages[$lang])) { $langsys = Sliced_UCDN::$ot_languages[$lang]; }
	else if ($lang!='' && $country !='' && isset(Sliced_UCDN::$ot_languages[$lang.''.$country])) { 
		$langsys = Sliced_UCDN::$ot_languages[$lang.''.$country]; 
	}
	else { $langsys = "DFLT"; }
