...
function CoonsPatchMesh($x, $y, $w, $h, $patch_array=array(), $x_min=0, $x_max=1, $y_min=0, $y_max=1, $colspace='RGB', $return=false){
	$s=' q ';
	$s.=sprintf(' %.3F %.3F %.3F %.3F re W n ', $x*_SLICED_MPDFK, ($this->mpdf->h-$y)*_SLICED_MPDFK, $w*_SLICED_MPDFK, -$h*_SLICED_MPDFK);
	$s.=sprintf(' %.3F 0 0 %.3F %.3F %.3F cm ', $w*_SLICED_MPDFK, $h*_SLICED_MPDFK, $x*_SLICED_MPDFK, ($this->mpdf->h-($y+$h))*_SLICED_MPDFK);
...
function Gradient($x, $y, $w, $h, $type, $stops=array(), $colorspace='RGB', $coords='', $extend='', $return=false, $is_mask=false) {
	...
	if ($type == 2) { 
		...
		$s = ' q';
		$s .= sprintf(' %.3F %.3F %.3F %.3F re W n', $x*_SLICED_MPDFK, ($this->mpdf->h-$y)*_SLICED_MPDFK, $w*_SLICED_MPDFK, -$h*_SLICED_MPDFK)."\n";
		$s .= sprintf(' %.3F 0 0 %.3F %.3F %.3F cm', $usew*_SLICED_MPDFK, $useh*_SLICED_MPDFK, $usex*_SLICED_MPDFK, ($this->mpdf->h-($usey+$useh))*_SLICED_MPDFK)."\n";
	}
