	...
	function svgText() {
		...
		if(isset($this->txt_data[2])) {
			...
			if ($this->mpdf_ref->useKerning) {
				...
				else { $textvar = ($textvar | SLICED_FC_KERNING); }
			}
						...
						if (_SVG_AUTOFONT && isset($attribs['lang']) && $attribs['lang']) { 
							if (!$svg_class->mpdf_ref->usingCoreFont) { 
								if ($attribs['lang'] != $svg_class->mpdf_ref->default_lang) {
									list ($coreSuitable,$mpdf_unifont) = sliced_mpdf_GetLangOpts($attribs['lang'], $svg_class->mpdf_ref->useAdobeCJK, $svg_class->mpdf_ref->fontdata);
...
function markScriptToLang($html) {
	...
	if (empty($this->script2lang)) {
		if (!empty($this->mpdf_ref->script2lang)) {
			...
			include(_SLICED_MPDF_PATH.'config_script2lang.php');
		}
	}
	   ...
	   if($i%2==0) {
		...
		for($sch=0;$sch<=$subchunk;$sch++) {
		   if (isset($chardata[$sch])) { 
			...
			if ($scriptblocks[$sch] == Sliced_UCDN::SCRIPT_LATIN && $this->mpdf_ref->autoVietnamese && preg_match("/([".$this->viet."])/u", $s)) {
				$lang="vi";
			}
			// Check Arabic for different languages if Arabic script - even if Basescript
			else if ($scriptblocks[$sch] == Sliced_UCDN::SCRIPT_ARABIC && $this->mpdf_ref->autoArabic) {
				...
				else if ($this->mpdf_ref->baseScript != Sliced_UCDN::SCRIPT_ARABIC  && isset($this->script2lang[$scriptblocks[$sch]])) {
