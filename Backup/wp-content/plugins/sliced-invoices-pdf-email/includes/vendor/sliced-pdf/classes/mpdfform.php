...
function print_ob_textarea($objattr,$w,$h,$texto,$rtlalign,$k,$blockdir) {
		   // TEXTAREA
			...
			$ClipPath = sprintf('q %.3F %.3F %.3F %.3F re W n ',$this->mpdf->x*_SLICED_MPDFK,($this->mpdf->h-$this->mpdf->y)*_SLICED_MPDFK,$w*_SLICED_MPDFK,-$h*_SLICED_MPDFK);
...
function print_ob_imageinput($objattr,$w,$h,$texto,$rtlalign,$k,$blockdir) {
		// INPUT/BUTTON as IMAGE
			...
			$this->mpdf->_out( sprintf("q %.3F 0 0 %.3F %.3F %.3F cm /I%d Do Q",$objattr['INNER-WIDTH'] *_SLICED_MPDFK,$objattr['INNER-HEIGHT'] *_SLICED_MPDFK,$objattr['INNER-X'] *_SLICED_MPDFK,($this->mpdf->h-($objattr['INNER-Y'] +$objattr['INNER-HEIGHT'] ))*_SLICED_MPDFK,$objattr['ID'] ) );
...
function _form_rect( $x, $y, $w, $h, $hPt ) {
	$x = $x * _SLICED_MPDFK;
	$y = $hPt - ($y  * _SLICED_MPDFK);
	$x2 = $x + ($w * _SLICED_MPDFK);
	$y2 = $y - ($h * _SLICED_MPDFK);
