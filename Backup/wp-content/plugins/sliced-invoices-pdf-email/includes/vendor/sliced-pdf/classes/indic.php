...
public static function set_indic_properties(&$info, $scriptblock ) {
	...
	*/
	...
	if ($cat == self::OT_Repha) {
		...
		*/
		if ($info['general_category'] == Sliced_UCDN::UNICODE_GENERAL_CATEGORY_NON_SPACING_MARK)
			$cat = self::OT_N;
	}
	...
	*/
	...
	if ((self::FLAG($cat) & (self::FLAG(self::OT_C) | self::FLAG(self::OT_CM) | self::FLAG(self::OT_Ra) | self::FLAG(self::OT_V) | self::FLAG(self::OT_NBSP) | self::FLAG(self::OT_DOTTEDCIRCLE)))) {	// = CONSONANT_FLAGS like is_consonant
		if ($scriptblock == Sliced_UCDN::SCRIPT_KHMER) $pos = self::POS_BELOW_C;	/* Khmer differs from Indic here. */
...
public static function set_syllables(&$o, $s, &$broken_syllables) {
	...
	while($ptr < strlen($s)) {
		...
		// From OT spec:
				...
				$o[$ptr - 1]['general_category'] < Sliced_UCDN::UNICODE_GENERAL_CATEGORY_LOWERCASE_LETTER || 
				$o[$ptr - 1]['general_category'] > Sliced_UCDN::UNICODE_GENERAL_CATEGORY_NON_SPACING_MARK
				)
...
public static function initial_reordering_syllable (&$info, $GSUBdata, $indic_config, $scriptblock, $is_old_spec, $start, $end) {
	...
	*/
	...
	if ($scriptblock != Sliced_UCDN::SCRIPT_KHMER) { 
...
*/
	...
	if ($is_old_spec && $scriptblock == Sliced_UCDN::SCRIPT_DEVANAGARI) {
...
public static function final_reordering_syllable (&$info, $GSUBdata, $indic_config, $scriptblock, $is_old_spec, $start, $end) {
	...
	*/
	...
	if ($start + 1 < $end && $start < $base) { 	/* Otherwise there can't be any pre-base matra characters. */
		...
		*/
		if ($scriptblock != Sliced_UCDN::SCRIPT_MALAYALAM && $scriptblock != Sliced_UCDN::SCRIPT_TAMIL) {
...
// ??? Need to TEST if actual substitution has occurred
	...
	if ($info[$start]['indic_position'] == self::POS_PRE_M &&
		($start==0 || 
		($info[$start - 1]['general_category'] < Sliced_UCDN::UNICODE_GENERAL_CATEGORY_FORMAT || $info[$start - 1]['general_category'] > Sliced_UCDN::UNICODE_GENERAL_CATEGORY_NON_SPACING_MARK)
		)) {
