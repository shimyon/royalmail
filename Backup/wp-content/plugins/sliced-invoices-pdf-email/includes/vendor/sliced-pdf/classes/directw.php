			...
			if($l > $wmax) {
				...
				if($sep == -1) {
					...
					if($align=='J') {
						...
						list($charspacing,$ws) = $this->mpdf->GetJspacing($nb_carac,$nb_spaces,((($w-2) - $len_ligne) * _SLICED_MPDFK),$inclCursive);
			...
			if($l > $wmax) {
				...
				if($sep == -1) {
					...
					if($align=='J') {
						...
						list($charspacing,$ws) = $this->mpdf->GetJspacing($nb_carac,$nb_spaces,((($w-2) - $len_ligne) * _SLICED_MPDFK),$false);
						$this->mpdf->SetSpacing($charspacing,$ws);
						//////////////////////////////////////////
					}
					$this->mpdf->Cell($w, $h, $tmp, 0, 2, $align, $fill, $link);
