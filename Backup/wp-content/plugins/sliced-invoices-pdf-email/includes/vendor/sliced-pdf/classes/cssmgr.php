...
function ReadCSS($html) {
	...
	if (count($idata[0])) { 
		for($i=0;$i<count($idata[0]);$i++) {
			$file = _SLICED_MPDF_TEMP_PATH.'_tempCSSidata'.RAND(1,10000).'_'.$i.'.'.$idata[2][$i];
			//Save to local file
	...
	if (strpos($CSSstr,'url(')!==false) {
		...
		for($i = 0; $i < count($m[1]) ; $i++) {
			...
			$CSSstr_copy = @preg_replace('/'.preg_quote($m[0][$i],'/').'/', 'url(\''.$tmp.'\')', $CSSstr);
			if ( $CSSstr_copy ) {
				$CSSstr = $CSSstr_copy;
			}
		}
...
/*-- END BORDER-RADIUS --*/
			...
			if (count($prop) == 2 ) {
				...
				if ($format) { $newprop['SHEET-SIZE'] = array($format[0]/_SLICED_MPDFK, $format[1]/_SLICED_MPDFK); }
			}
		}
...
function inlinePropsToCSS($bilp, &$p) {
	...
	if (isset($bilp[ 'textvar' ]) && $bilp[ 'textvar' ]) {
		...
		if ($bilp[ 'textvar' ] & SLICED_FD_LINETHROUGH) { 
			if ($bilp[ 'textvar' ] & SLICED_FD_UNDERLINE) { $p['TEXT-DECORATION'] = 'underline line-through'; }
			else { $p['TEXT-DECORATION'] = 'line-through'; } 
		}
		else if ($bilp[ 'textvar' ] & SLICED_FD_UNDERLINE) { $p['TEXT-DECORATION'] = 'underline'; }
		else { $p['TEXT-DECORATION'] = 'none'; }

		if ($bilp[ 'textvar' ] & SLICED_FA_SUPERSCRIPT) { $p['VERTICAL-ALIGN'] = 'super'; }
		else if ($bilp[ 'textvar' ] & SLICED_FA_SUBSCRIPT) { $p['VERTICAL-ALIGN'] = 'sub'; }
		else { $p['VERTICAL-ALIGN'] = 'baseline'; }

		if ($bilp[ 'textvar' ] & SLICED_FT_CAPITALIZE) { $p['TEXT-TRANSFORM'] = 'capitalize'; }
		else if ($bilp[ 'textvar' ] & SLICED_FT_UPPERCASE) { $p['TEXT-TRANSFORM'] = 'uppercase'; }
		else if ($bilp[ 'textvar' ] & SLICED_FT_LOWERCASE) { $p['TEXT-TRANSFORM'] = 'lowercase'; }
		else { $p['TEXT-TRANSFORM'] = 'none'; }

		if ($bilp[ 'textvar' ] & SLICED_FC_KERNING) { $p['FONT-KERNING'] = 'normal'; }	// ignore 'auto' as default already applied
		//if (isset($bilp[ 'OTLtags' ]) && $bilp[ 'OTLtags' ]['Plus'] contains 'kern'
		else { $p['FONT-KERNING'] = 'none'; }

		if ($bilp[ 'textvar' ] & SLICED_FA_SUPERSCRIPT) { $p['FONT-VARIANT-POSITION'] = 'super'; }
		//if (isset($bilp[ 'OTLtags' ]) && $bilp[ 'OTLtags' ]['Plus'] contains 'sups' / 'subs'
		else if ($bilp[ 'textvar' ] & SLICED_FA_SUBSCRIPT) { $p['FONT-VARIANT-POSITION'] = 'sub'; }
		else { $p['FONT-VARIANT-POSITION'] = 'normal'; }

		if ($bilp[ 'textvar' ] & SLICED_FC_SMALLCAPS) { $p['FONT-VARIANT-CAPS'] = 'small-caps'; }
	}
