...
// First clear any files in directory that are >1 hrs old
	$interval = 3600;
	if ($handle = opendir(_SLICED_MPDF_PATH.'graph_cache')) {
	   while (false !== ($file = readdir($handle))) { 
		if (((filemtime(_SLICED_MPDF_PATH.'graph_cache/'.$file)+$interval) < time()) && ($file != "..") && ($file != ".")) { 
			@unlink(_SLICED_MPDF_PATH.'graph_cache/'.$file); 	// mPDF 4.0
		}
	   }
	   closedir($handle); 
	}
...
// LOAD GRAPHS
	...
	include_once(_SLICED_JPGRAPH_PATH.'jpgraph.php'); 
	include_once(_SLICED_JPGRAPH_PATH.'jpgraph_line.php' ); 
	include_once(_SLICED_JPGRAPH_PATH.'jpgraph_log.php' ); 
	include_once(_SLICED_JPGRAPH_PATH.'jpgraph_scatter.php' ); 
	include_once(_SLICED_JPGRAPH_PATH.'jpgraph_regstat.php' ); 
	include_once(_SLICED_JPGRAPH_PATH.'jpgraph_pie.php'); 
	include_once(_SLICED_JPGRAPH_PATH.'jpgraph_pie3d.php'); 
	include_once(_SLICED_JPGRAPH_PATH.'jpgraph_bar.php'); 
	include_once(_SLICED_JPGRAPH_PATH.'jpgraph_radar.php'); 
...
// mPDF 2.5 
	...
	if ($graph) {
		$graph->Stroke( _SLICED_MPDF_PATH.$figure_file);
