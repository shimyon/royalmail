...
class fpdi_pdf_parser extends pdf_parser {
    ...
    function _rebuildContentStream($obj) {
        ...
        foreach ($filters AS $_filter) {
			...
			// mPDF 4.2.003
                case '/LZWDecode': 
			include_once(_SLICED_MPDF_PATH.'mpdfi/filters/FilterLZW.php');
			// mPDF 5.0 Removed pass by reference =&
			$decoder = new FilterLZW();
			$stream = $decoder->decode($stream);
			break;
                case '/ASCII85Decode':
			include_once(_SLICED_MPDF_PATH.'mpdfi/filters/FilterASCII85.php');
			// mPDF 5.0 Removed pass by reference =&
    ...
    function getPageBox($page, $box_index) {
        ...
        if (!is_null($box) && $box[0] == PDF_TYPE_ARRAY) {
            $b =& $box[1];
            return array("x" => $b[0][1]/_SLICED_MPDFK,
                         "y" => $b[1][1]/_SLICED_MPDFK,
                         "w" => abs($b[0][1]-$b[2][1])/_SLICED_MPDFK,
                         "h" => abs($b[1][1]-$b[3][1])/_SLICED_MPDFK);	// mPDF 5.3.90
